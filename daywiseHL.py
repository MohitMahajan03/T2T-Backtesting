...
def highest_lowest(i, ind_high, ind_low, days):
    days_highs = []
    days_lows = []
    for j in range(0, days):
        days_highs.append(ind_high[i-j])
        days_lows.append(ind_low[i-j])
    high = max(days_highs)
    low = min(days_lows)
    # print("1", days_highs)
    print(prev_hl)
...
def report(bep, index, lot_size):
    ...
    # excel_df.to_csv("report_high-low1.csv")
...
def run(info):
    ...
    print(index[0], start_date, end_date)
    if((index[0] == "NIFTY" or index[0] == "BANKNIFTY" or index[0] == "FINNIFTY") and index[1] == "EQ"):
        ...
        if index[0] in ind_eq.keys():
            ...
            instrument = "FUTSTK"
        print(index[0], instrument, start_date, end_date)
    ...
    for j in range(days - 1, len(ind_vals)):
            try:  
                highest_lowest(j, ind_high, ind_low, days)
                ...
                trade(bep, line, entry_buff, exit_buff, ind_history, ind_vals[j+1],ind_open[j+1],ind_high[j+1],ind_low[j+1],ind_close[j+1], j+1, fig, ind_date, msl)
            except Exception as e:
    ...
    return excel_df
# print(trades)
# info = ("ALUMINIUM COM", "Close", 0.225, 0.225, 3, 2.25, "no", "2021-01-01", "2021-01-31")
# run(info)